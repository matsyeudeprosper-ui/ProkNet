...
| v0.18.2 | Two review gaps closed: a server-enforced pilot allowlist (`PROK_PILOT_IDS`) so `PROK_PAYMENTS_LIVE` opens real money only to listed identities - unlisted ones see no treasury number and are never credited automatically, even from a valid message; and no approval without a same-day typed balance on that rail that is not below the ledger (missing / stale / low / matching each tested) | built; server 437 tests; Android totals in the release; **no phone test run**; real payments OFF |
| v0.18.1 | The v0.18 release, complete: contract v3 with one charging rule on both phones and the Brain (nothing down, nothing owed; from the first byte otherwise) and the relay in the signed contract, paid 10 % (example) when the session settles; a paid session needs a confirmed hold - no Brain, no paid session, free sessions untouched; the kiosk / direct-to-seller route off in the product; refunds to the origin number, reversals with a memo, identity recovery after a reinstall, top-up caps, per-rail reconciliation that blocks approvals on doubt; schema 6; restore/rollback script; the acceptance list TESTING 81 and docs/HANDOFF_V018.md | built; server and Android suites pass (numbers in the handoff); **no phone test run**; real payments OFF |
| v0.18.0 | The Prok ledger on the Brain (schema 5): credit, holds that stay reserved while a session may still settle, earnings from verified evidence, withdrawal on request, a treasury queue whose first line is "N retraits en attente = N envois manuels", and operator-message matching with unmatched payments held for review. No operator API; real customer payments disabled (`PROK_PAYMENTS_LIVE` off) | built, 760 JVM + 405 server tests pass; hardware acceptance pending (TESTING 80) |
| v0.17.11 | The pilot Brain address is the default, so a reset phone is on the network without anybody typing; a public http:// address is refused | built, 739 JVM + 360 server tests |
