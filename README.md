...
| v0.14.0 | Human pricing: the buyer picks a CFA budget and can never be charged more; the seller picks what to earn and never types a price; ProkNet computes a rate that covers the seller's real data cost, safety and margin, keeps free Internet free, and refuses deals that would lose money | built, 285 JVM + 11 server tests pass; budget-session hardware test pending |
