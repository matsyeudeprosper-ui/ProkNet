...
| v0.12.1 | The design pass: one design system (colors light + night, type scale, bg_* drawables), a glowing one-tap button, bordered rounded cards, readable legend pills, status chip with a dot, a real nav bar; the map card no longer contradicts itself | built, 236 JVM tests pass, one-tap hardware regression pending |
