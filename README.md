...
| v0.9.14 | Client membership is part of the transport: a data plane generation only becomes usable when somebody has joined, the listener is armed for THAT membership, the provider announces TRANSPORT_READY and dials the customer as well, and every socket says how it is bound | built, 151 JVM tests pass |
