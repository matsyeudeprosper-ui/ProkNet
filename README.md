...
| v0.14.1 | Stable budget contracts: signed contracts are parsed at the length their own version byte declares, the admission decision is one pure function the tests run, the budget ceiling holds against hostile byte counts, and the whole session is tested from proposal to ledger | built, 310 JVM + 11 server tests pass; hardware acceptance pending (TESTING 59) |
